...
X <- as.data.frame(X[, 3:4])
X <- na.omit(X)

# Add column names
V0 <- with(X, table(`temps travail`, Fonction))
row_names <- c(
  "la charge de travail correspond à mes horaires de travail",
  "je ne dépasse pas mes horaires de travail",
  "je suis en sous charge de travail",
  "je suis surchargé et je dépasse mes horaires de travail",
  "Non répondu"
)
col_names <- c(
...
rownames(V0) <- row_names
colnames(V0) <- col_names
...
rownames(Dn) <- row_names
colnames(Dn) <- row_names

Dp <- diag(J)

rownames(Dp) <- col_names
colnames(Dp) <- col_names
...
plot(C1, C2, pch = 24, cex = 1, col = "blue", lwd = 2, xlim = c(-0.15, 0.05), ylim = c(-0.05, 0.05))
