X <- matrix(c(10, 0, 0, 0, 9, 3, 0, 1, 7), nrow = 3, ncol = 3)

# Add column names
colnames(X) <- c("Percu Sucré", "Percu Amide", "Percu Amer")

# Add row names
rownames(X) <- c("Sucré", "Amide", "Amer")

# If you want to create a table from the matrix
V0 <- as.table(X)
...
rownames(Dn) <- c("Sucré", "Amide", "Amer")
colnames(Dn) <- c("Sucré", "Amide", "Amer")

Dp <- diag(J)

rownames(Dp) <- c("Percu Sucré", "Percu Amide", "Percu Amer")
colnames(Dp) <- c("Percu Sucré", "Percu Amide", "Percu Amer")
...
v1 <- sqrt(Dn) %*% eigen_vectors2[, 1]
v2 <- sqrt(Dn) %*% eigen_vectors2[, 2]

C1_2 <- t(V) %*% v1
C2_2 <- t(V) %*% v2

plot(C1_2, C2_2, type = "p", col = "blue")
text(C1_2, C2_2, colnames(V), cex = 0.7, pos = 3, col = "blue")
abline(h = 0, col = "gray")
abline(v = 0, col = "gray")

# All points in the same plot
plot(C1, C2, pch = 24, cex = 1, col = "blue", lwd = 2)
