...
f_k <-
  function(k, lbound, ubound) {
    k / k / (ubound - lbound)
  }  # "k/k" : dirty trick to get a sequence of constant numbers.
f_u <- function(k, lbound, ubound) {
  1 / k ^ 2 / (1 / lbound - 1 / ubound)
}
f_w <-
  function(k, lbound, ubound) {
    k ^ (-1) / (log(10) * (log10(ubound) - log10(lbound)))
  }
...
integrate_density_M2 <- function(k,
                                 density_k,
                                 lb = 0,
                                 ub = 1E+99)
  ...
  # Plot the 3 priors. Returns the integrals of the 3 priors.
{
  ...
  int <-
    integrate_3densities_M2(priors) # We integrate first the three priors.
  ...
  p = ggplot() +
    ...
    xlab(expression(paste("k(.", s ^ {
      -1
    }, ")"))) +
    ylab('Probability density') + scale_x_continuous(trans = 'log10', labels = scales::scientific) +
    scale_y_continuous(trans = 'log10', labels = scales::scientific) +
    scale_color_discrete(name = "Prior",
                         labels = c(expression(f[k]), expression(f[u]), expression(f[w])))#+        coord_cartesian(xlim =c(1E+04,1E+10))
...
plot_profile_M2 <- function(t_model, R_model)
{
  ...
  R_profile_exp <-
    read.table("R_Exp.csv", header = FALSE) # We first extract the measurements.
  ...
  p = ggplot() +
    geom_point(data = R_profile_exp, aes(
      x = t,
      y = R,
      color = "blue"
    )) +
    geom_line(data = R_profile_model, aes(
      x = t,
      y = R,
      color = "black"
    )) +
    xlab(expression(paste("t(", mu, "s", ")"))) +
    ylab(expression(paste("[R] (mol.", cm ^ {
      -3
    }, ")"))) +
    scale_color_discrete(name = "Profile",
                         labels = c("Optimal profile", "Experiment")) +
...
Compute_likelihood_M2 <- function(R_profile_exp, R0, k, epsilon) {
  ...
  R <-
    Compute_R_profile_M2(t_exp, R0, k)   # Profile predicted by k at the
...
Examine_likelihood_M2 <-
  function(R0, lbound, ubound, n_k, epsilon = 0.065)
  ...
  # which must be extracted from the data frames before calling that function.
{
  ...
  posterior <-
    rep(1, n_k)   # Declaration and initialisation of the vector.
  ...
  for (i in 1:n_k) {
    ...
    if (2 <= i)  {
      normalisation =  normalisation + 0.5 * (posterior[i - 1] + posterior[i]) *
        (k[i] - k[i - 1])
    }
  }  # End for
...
Compute_all_posteriors_M2 <-
           ...
           ub_plot)
  {
    ...
    max_likelihood <-
      Compute_likelihood_all_M2(R0, lbound, ubound, n_k, epsilon)
    ...
    p = ggplot() +
      ...
      scale_color_discrete(name = "Posterior",
                           labels = c(expression(f[k]), expression(f[u]), expression(f[w]))) +
      xlab(expression(paste("k (", s ^ {
        -1
      }, ")"))) +
